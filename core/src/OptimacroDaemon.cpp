...
const string pathToFile = "./savedCodes.json";
...
vector<Macro> loadMacros(const string &jsonFilePath) {
  ...
  while ((pos = jsonContent.find("{", pos)) != string::npos) {
    ...
    if (namePos != string::npos && shortcutPos != string::npos &&
        filePathPos != string::npos) {
      Macro macro;
      macro.name = jsonContent.substr(
          namePos + 9, jsonContent.find("\"", namePos + 9) - (namePos + 9));
      macro.shortcut = jsonContent.substr(
          shortcutPos + 13,
          jsonContent.find("\"", shortcutPos + 13) - (shortcutPos + 13));
      macro.filePath = jsonContent.substr(
          filePathPos + 13,
          jsonContent.find("\"", filePathPos + 13) - (filePathPos + 13));
...
void registerKeys(Display *display, const vector<Macro> &macros) {
  for (const auto &macro : macros) {
    ...
    if (keyCode != 0) {
      XGrabKey(display, keyCode, modifiers, DefaultRootWindow(display), True,
               GrabModeAsync, GrabModeAsync);
      registeredShortcuts.insert(
          macro.shortcut); // Dodajemy do zarejestrowanych skrótów
    }
  }
}
...
void unregisterKeys(Display *display, const vector<Macro> &previousMacros) {
  for (const auto &macro : previousMacros) {
    ...
    KeyCode keyCode = XKeysymToKeycode(display, XStringToKeysym(key.c_str()));
    if (keyCode != 0 &&
        registeredShortcuts.find(macro.shortcut) != registeredShortcuts.end()) {
      XUngrabKey(display, keyCode, modifiers,
                 DefaultRootWindow(display)); // Odrejestrowujemy skrót
...
void monitorFileChanges(vector<Macro> &macros, const string &pathToFile,
  ...
  while (true) {
    ...
    if (currentModificationTime != lastModificationTime) {
      ...
      for (const auto &macro : macros) {
        cout << "Loaded macro: " << macro.shortcut << " (" << macro.filePath
             << ")" << endl;
      }
    }
  ...
  while (true) {
    while (XPending(display)) {
      ...
      if (event.type == KeyPress) {
        ...
        cout << "Key pressed: " << pressedKey << " (Ctrl: " << ctrlPressed
        ...
        for (const auto &macro : macros) {
          ...
          if (pressedKey ==
              macroShortcut.substr(macroShortcut.find_last_of("+") + 1)) {
            if ((macroShortcut.find("ctrl") != string::npos && ctrlPressed) &&
              ...
              matchFound = true;
            } else if ((macroShortcut.find("ctrl") == string::npos ||
                        ctrlPressed) &&
                       (macroShortcut.find("alt") == string::npos ||
                        altPressed) &&
                       (macroShortcut.find("shift") == string::npos ||
                        shiftPressed)) {
              matchFound = true;
            }
          }
...
int runDaemon() {
  try {
    ...
    thread fileMonitorThread(monitorFileChanges, ref(macros), pathToFile,
                             display);
